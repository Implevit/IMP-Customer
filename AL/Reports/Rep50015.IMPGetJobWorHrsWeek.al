report 50015 "IMP Get Job Wor. Hrs. Week"
{
        ...
        dataitem(Integer; Integer)
        {
            dataitem(Resource; Resource)
            {
                dataitem("IMP Job Types"; "IMP Job Types")
                {
                    ...
                    var
                        ...
                        l_JobWorkingHours: Record "IMP Job Res. Working Hours";
                        l_Year: Integer;
                ...
                var
                    l_JobWorkingHours: Record "IMP Job Res. Working Hours";
                begin
                    l_JobWorkingHours.SETRANGE(l_JobWorkingHours."Week Start", g_Date);
            ...
            begin
                IF g_Date = 0D THEN BEGIN
                    g_Date := DWY2DATE(1, Number, g_Year);
                    g_LastDate := CALCDATE('<1W>', g_Date);
                    g_LastDate := CALCDATE('<-1D>', g_LastDate);
                END ELSE BEGIN
                    g_Date := CALCDATE('<1W>', g_Date);
                    g_LastDate := CALCDATE('<1W>', g_Date);
                    g_LastDate := CALCDATE('<-1D>', g_LastDate);
                END;
